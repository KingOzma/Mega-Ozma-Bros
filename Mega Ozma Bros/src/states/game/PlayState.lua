...
-- Render score
    ...
    love.graphics.setColor(0, 0, 0, 255)
    love.graphics.print("Level - " .. tostring(self.player.currentLevel), 5, SUPER_HEIGHT - 15)
    love.graphics.setColor(255, 255, 255, 255)
    love.graphics.print("Level - " .. tostring(self.player.currentLevel), 4, SUPER_HEIGHT - 16)
end

function PlayState:updateCamera()
-- Clamp movement of the camera's X between 0 and the map bounds -  width,
-- Setting it half the screen to the left of the player so they are in the center
    self.camX = math.max(0,
        math.min(TILE_SIZE * self.tileMap.width - SUPER_WIDTH,
        self.player.x - (SUPER_WIDTH / 2 - 8)))
